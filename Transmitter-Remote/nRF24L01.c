...
void nRF_Init(char mode, char rf_channel) {
    ...
    nRF_WriteRegister(CONFIG_N, 0x0E); // Power Up, CRC Enable, CRC 2 byte
    __delay_ms(10);
    nRF_WriteRegister(EN_AA, 0x00); // Enable RX Address (data pipe 0)
...
void SPI_Write(char data) {
    SSP2BUF = data;           // Copy data in SSBUF to transmit
    while(!PIR3bits.SSP2IF);     // Wait for complete 1 byte transmission
    PIR3bits.SSP2IF = 0;         // Clear SSP2IF flag
    data = SSP2BUF;         // Clear SSP2IF flag
}

char SPI_Read() {
    SSP2BUF = 0x00;           // Copy data in SSBUF to transmit
    while(!PIR3bits.SSP2IF);     // Wait for complete 1 byte transmission
    PIR3bits.SSP2IF = 0;         // Clear SSP2IF flag
    return SSP2BUF;
}
...
void nRF_SetMode(char mode) {
    ...
    if(mode) {
        nRF_WriteRegister(CONFIG_N, 0x0F);  // RX Control
        CE_RF = 1;  // RX Mode
    } else {
        nRF_WriteRegister(CONFIG_N, 0x0E);  // TX Control
        CE_RF = 0;  // TX Mode
    }
}
...
void nRF_StandByI() {
    nRF_WriteRegister(CONFIG_N, 0x0A);
    __delay_ms(10);
}